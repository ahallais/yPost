...
func TestBatchedRecoverySpansFilesAndMultipleBatches(t *testing.T) {
	tempDir := t.TempDir()
	sliceSize := 64
	dataShards := recoveryDataShardsPerBatch*2 + 44
	data := make([]byte, dataShards*sliceSize-17)
	for i := range data {
		data[i] = byte(i*31 + 7)
	}

	// Deliberately split in the middle of logical shards. The encoder must
	// treat the files as one continuous input stream.
	cut := recoveryDataShardsPerBatch*sliceSize + 23
	paths := []string{
		filepath.Join(tempDir, "part.001"),
		filepath.Join(tempDir, "part.002"),
	}
	if err := os.WriteFile(paths[0], data[:cut], 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(paths[1], data[cut:], 0644); err != nil {
		t.Fatal(err)
	}

	recovery, err := NewGenerator(tempDir).generateRecoveryDataBatched(paths, sliceSize, 10, "test")
	if err != nil {
		t.Fatal(err)
	}
	...
	wantBlocks := 30
	if recovery.blocks != wantBlocks {
		t.Fatalf("recovery blocks = %d, want %d", recovery.blocks, wantBlocks)
	}
	info, err := recovery.file.Stat()
	if err != nil {
		t.Fatal(err)
	}
	if wantSize := int64(wantBlocks * sliceSize); info.Size() != wantSize {
		t.Fatalf("recovery spool size = %d, want %d", info.Size(), wantSize)
	}
	contents, err := os.ReadFile(recovery.file.Name())
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Equal(contents, make([]byte, len(contents))) {
		t.Fatal("recovery spool unexpectedly contains only zeroes")
	}
}
